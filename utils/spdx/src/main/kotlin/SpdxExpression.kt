...
class SpdxCompoundExpression(
    ...
    private fun replaceSubexpressionWithChoice(subExpression: SpdxExpression, choice: SpdxExpression): SpdxExpression {
        // If the operator is AND and the subexpression is contained in one of the children (not the whole expression),
        // then there is no choice at this level but try with the children.
        if (operator == SpdxOperator.AND && this != subExpression && children.any { it.isSubExpression(subExpression) }) {
